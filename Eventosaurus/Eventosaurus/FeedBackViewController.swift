...
class WriteFeedbackViewController: UIViewController {
    ...
    @IBAction func submitFeedbackButton(_ sender: Any) {
        ...
        if feedbackDescription.isEmpty {
            ...
            if let eventID = eventID {  // Ensure eventID is not nil
                saveFeedback(description: feedbackDescription, stars: starRating, eventID: eventID) { [weak self] in
                    // After successfully saving the feedback, present the EventHomeViewController
                    
                    // Optionally reset the text view after submission
                    self?.FeedBackTxt.text = ""
                    
                    // Create an instance of EventHomeViewController
                    let storyboard = UIStoryboard(name: "HomePage ", bundle: nil) // Adjust the storyboard name if necessary
                    if let eventHomeVC = storyboard.instantiateViewController(withIdentifier: "EventHomeViewController") as? EventHomeViewController {
                        // Present EventHomeViewController modally
                        eventHomeVC.modalPresentationStyle = .fullScreen // Use .fullScreen for full-screen modal presentation
                        self?.present(eventHomeVC, animated: true, completion: nil)
                    }
                }
            }
        }
    ...
    func saveFeedback(description: String, stars: Int, eventID: String, completion: @escaping () -> Void) {
        let db = Firestore.firestore()
        ...
        let feedbackData: [String: Any] = [
            ...
            "Stars": stars,
            "EventID": eventRef,  // Store the EventID as a reference to the Event document
        ]
        ...
        db.collection("FeedBack").addDocument(data: feedbackData) { error in
            if let error = error {
                ...
                // Call the completion handler once the feedback is saved
                completion()
            }
        }
    }
