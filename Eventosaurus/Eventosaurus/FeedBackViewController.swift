...
class WriteFeedbackViewController: UIViewController, EventSelectionDelegate {
    ...
    // eventID can be passed, but we are hardcoding it to a fixed value in saveFeedbackAndRating
    var eventID: String? {
            didSet {
                print("Event ID set in WriteFeedbackViewController: \(eventID ?? "None")")
            }
        }
        ...
        override func viewDidLoad() {
            ...
            // If delegate is set properly, eventID will be accessible here
            if let eventID = (delegate as? EventSelectionDelegate)?.eventID {
       ...
       func saveFeedbackAndRating(description: String, stars: Int) {
           ...
           guard let eventID = EventCurrentSelection.shared.getEventID() else {
                  print("No Event ID passed")
                  return
              }
